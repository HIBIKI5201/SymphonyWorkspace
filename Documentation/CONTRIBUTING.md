...
### 作業中に見つけた不具合は、元のIssueへ返信する

**別Issueの確認や作業の途中で不具合を見つけたら、その場でチャットへ報告して終わらせず、由来するIssueへコメントを残してください。**

- **由来するIssueがある場合**（そのIssueの対応で作り込まれた欠陥、確認を依頼された項目の結果）は、**そのIssueへ返信する**。新しいIssueを立てない。**確認の依頼と結果が同じ場所に並ぶことに意味がある**
- **由来するIssueが無い場合**は新しいIssueを立てる
- **直したかどうかに関わらず書く。** 直したなら何をどう直したか、直していないなら再現手順と実測値を書く
- **確認を依頼されていた項目は、確認できなかったものも書く。** 環境の都合で確認できない項目を黙って飛ばすと、確認済みだと誤解されたままIssueが閉じられる
- **クローズの可否を明示する。** 未解決の項目が残っているなら「まだクローズできない」と書く

チャットの報告は会話が終われば消えます。**次に同じ場所を触る人が読むのはIssueであって、会話ログではありません。**

実際に、Issue #215 の確認作業で「索引のGitHubフォールバックが `index.md` 追加前の前提のまま」という欠陥と、「生成器の更新が親リポジトリへ入っておらず preflight が誰の環境でも止まる」という欠陥を見つけています。どちらも #215 の対応そのものに由来するため、#215 へ返信しています。
