...
3. **コンパイル** — `uloop-clear-console` → `uloop-compile` → `uloop-get-logs`。エラー0件、意図しない警告なし。
   ...
   **テストの件数が Round の前より増えている（または変更されている）ことを確認する。** 本体を変えてテストが1件も動いていない Round は、`release_round.py preflight` で止まる。
...
### 一覧を返す Query は順序を決めているか

**複数件を返す Query（`IReadOnlyList`、配列、`IEnumerable`）を追加・変更したら、その順序が何で決まるかを差分から読み取れることを確認する。**

順序を決めていない実装はコンパイルもテストも通る。壊れるのは表示側で、ListView の行が更新のたびに入れ替わる、差分比較が毎回「変わった」と判定して再描画が止まらない、といった形で後から出る。**Unity を開けない環境では、これが最も見つからない種類の欠陥である。**

見るのは次の2つ。

- 元になっているコレクションが `Dictionary` や `HashSet`、あるいはそれらの和集合なら、**列挙順は保証されない。** 明示的に並べ替えているか
- 並べ替えているなら、その基準が実行をまたいで安定するか。`GetHashCode` や参照の並びではなく、名前や登録順のように再現する値か

実際に PauseManager のカテゴリー一覧は state と registry の和集合から作っており、`CategoryName` の序数比較で並べ替えて確定させた。**Scene Block の一覧も同じ構造を持つ。**
