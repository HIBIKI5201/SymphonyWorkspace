...
| 3 | 指定モデルがアカウントで使えない | **ファイルは変更されていない。** `--model` で使えるモデルを指定して再実行する |

`exit 3` は `~/.codex/config.toml` の既定モデルがアカウントの契約で使えない場合に出る。実際に、既定が `gpt-5.6-sol` のまま無料プランで実行して 400 を受けた事例がある。APIへ到達した直後の失敗なので作業ツリーは無傷で、モデルを変えて再実行すればよい。**`exit 1` と混同して `git status` の確認から始めない。**
...
- **テストの実装は成果物に含める。** 本体のソースを変更したら、同じ Round で `Tests/` へテストを追加または変更する。`release_round.py preflight` が機械的に検査するため、抜けているとコミットできない
...
### 複数Roundにまたがる作業は、着手前に残量を見積もる

**Round 分割を書いた時点で `--check-only --json` を1回実行し、全Roundをワーカーで回せるかを見る。** 残量はRoundごとに減っていくため、途中で尽きると分割の前提が崩れる。

実際に、5 Round の計画を立てて Round 1・2 をワーカーへ委譲したところ、Round 3 の開始時に残量が閾値を割り、残る3 Roundを自分で実装することになった事例がある。**着手前に見ていれば、最初から自分で実装する前提で見積もれた。**

判断の目安は次のとおり。`resets_at` までの時間と併せて決める。

| 状況 | 扱い |
| --- | --- |
| 残量が十分にあり、リセットも近い | 予定どおりワーカーへ委譲する |
| 全Roundを回すには足りないが、リセットが数時間後 | 足りる分だけ委譲し、残りはリセットを待つ |
| リセットが日単位で先 | **最初から自分で実装する前提で計画する。** 途中で切り替えると、Roundごとの進め方が揃わない |
...
#### ドキュメントを再編するRound用の追加制約

既存文書を別ファイルへ移す Round では、上のテンプレートへ次も含める。**「要約・言い換えするな」だけを渡すと、ワーカーは忠実に写す代わりにそこで止まる。**

```text
- **文章を要約・言い換えしない。** 既存の文面をそのまま移す。移送先で文脈が繋がらない箇所だけ、接続の1文を足す。
- **移送元から消し忘れない。** 同じ説明が2か所へ残ると、次に直すとき片方が取り残される。
- **1つの節に複数の機能が入る場合は `###` で分ける。** 見出し無しで連結しない。
- **移送元の説明が1行しか無い型・機能は、ソースのXMLドキュメントを読んで最小限の用途を補う。**
  入口の表だけの文書を作らない。ただしソースに書かれていないことを推測で書かない。
- 相対リンクを張り替え、**リンク先とアンカーが実在することを確認する。**
```

実際に、この2項目が無かった Round で次が起きている。

- README の箇条書き1行しか説明が無かった `SymphonyStopWatch` が、移送先で入口の表だけの文書になり、元の1行（「ID単位の簡易処理時間計測」）は移送元から消えて**どこにも残らなかった**
- 3つの独立したEditor機能が、見出し無しで1つの節へ連結された
