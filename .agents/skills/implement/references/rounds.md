...
- **公開型を1つ増やすたびにテストファイルが1つ増える。** `PublicTypeTestCoverageTests` が「公開型 `X` に対して `Tests/Editor/XTests.cs` があるか」を検査するため、公開型はenumや例外も含めて1つずつテストファイルを要求する。**Round の大きさは、実装するファイル数より公開型の数でほぼ決まる。** 分割を決める前に、その Round で `public` にする型を数えること
