...
- `Documentation/DesignPhilosophy.md` — レイヤー、依存方向、公開範囲の判断基準。次の4箇所を読む
  - `## クラス設計` と `## 公開APIとバージョニング`
  - **新設・変更する型が属するレイヤーの `### <レイヤー名>` 節**（`### ViewModel`、`### Query`、`### Service` など）。レイヤーごとの禁止事項はここにしか書かれていない
  - **`## やってはいけないこと` の全項目**。上から下まで通す。該当しそうな行だけを拾わない
- `Documentation/CodeGuidelines.md` `## 名前空間` — 配置先の決定に必要

**レイヤー節と `## やってはいけないこと` を飛ばすと、設計が禁止事項そのものになる。** 実際に、Issue #166 の設計書が「Editor Window の操作を ViewModel へ集約する」と書いて確定した後、`### ViewModel` の「Commandを実行しない。Runtime ViewやEditor Windowの操作は公開エントリポイントを直接呼ぶ」と、`## やってはいけないこと` の「ViewModelからCommandを実行する」に真正面から抵触していることがワーカーの指摘で判明した。**`## 公開範囲` だけを読んでいた**ためで、ワーカーが1回実装を終えた後の配線やり直しになっている。
...
**ただし、操作できないことと確認できないことは別である。** 開いてスクリーンショットを撮ること、UI Toolkit の表示要素の実値（ラベルの文字列、`ListView.itemsSource.Count`）を読むこと、設定値を切り替えて追従を見ることはできる。**手順は [review.md](review.md) の「Editor UI は操作できなくても確認できる」にある。** 「動作確認手順」で人へ回す項目を決める前に、そちらで確認できる範囲を先に見積もること。

したがって、**GUI の押下を含む Round は設計の時点で「人の確認が要る」と分かる**。次を設計書へ書く。
...
- GUI を経由しない公開APIがあるなら、それを通した結果（出力ファイルの中身など）で
  ロジック側を裏取りする手順も併記する
- **「設定やアセットを変更したとき、開いたままのウィンドウが追従するか」を項目として書く。**
  ウィンドウを開き直せば直る不具合は、開き直してから確認すると見つからない。
  実際に、Project Settings の配列を変更しても開いたままのウィンドウが古い一覧を表示し続ける実装を、
  たまたまその状態でスクリーンショットを撮ったために見つけている（Issue #124 Round 2）。
  手順に項目が無いと、気づくかどうかが確認の順番任せになる
...
### Round の区分と、追加する検証の強さを突き合わせる

**「公開APIを変えない Round」に例外を1つ追加すると、それだけで破壊的変更になる。** 今まで通っていた呼び出しが例外で落ちるなら、シグネチャが同じでも利用側は壊れる。**区分（Fix / Change / 追加）と、その Round で追加する検証の強さは、設計の時点で突き合わせる。**

- 新しい検証を「例外にする」か「既定値へ倒す」かは、**その Round の区分が決める。** 内部だけの Round なら既定値へ倒す。例外にするなら破壊的変更の Round へ回す
- 既存の実装・テスト・サンプルがその検証に引っかからないかを、着手前に列挙して確認する。引っかかるものが1つでもあるなら、それは公開APIの変更である

実際に Issue #168 では「カテゴリーを指定しない登録は `ArgumentException`」と設計したが、`IPausable` を直接実装している既存のテストとサンプルが全て落ちる破壊的変更であり、公開APIを変えない Round には載せられなかった。**着手後に既定カテゴリーへ倒す設計へ変えている。**
...
**検証の結果、Issueの前提が崩れた場合は Issue へ書く。** 起票時の想定と現実が違うことは、起票者が知る必要がある。書き方は [issue-dialogue.md](issue-dialogue.md) にある。
